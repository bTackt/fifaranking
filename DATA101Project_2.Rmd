...
head(mfifa)
tail(mfifa)
unique(mfifa$rank_date) #oh my we have a lot of snapshots by time
````
...
mfifa %>% group_by(country_full) %>% 
        mutate(x=1:n()) %>%
        ggplot(aes(x=rank_date, y=rank,group=country_full, color=country_full)) + geom_line() + theme(legend.position="none") + scale_y_reverse()
````
...
conmebol %>% group_by(country_full) %>% 
        mutate(x=1:n()) %>%
        ggplot(aes(x=rank_date, y=rank,group=country_full, color=country_full)) + geom_line() + theme(legend.position="none") + scale_y_reverse()
...
concacaf %>% group_by(country_full) %>% 
        mutate(x=1:n()) %>%
        ggplot(aes(x=rank_date, y=rank,group=country_full, color=country_full)) + geom_line() + theme(legend.position="none") + scale_y_reverse()
